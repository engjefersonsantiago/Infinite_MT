...
In our analysis, we define a flow as a unique five tuple $\langle$~IPv4/IPv6 Source IP address, IPv4/IPv6 destination IP address, L4 protocol, UDP/TCP source port, UDP/TCP destination port~$\rangle$ connection.
...
On average, short flows ($\sim$100 bytes) dominate the trace with 75\% of the flows are no bigger than 150 bytes.

According to our experiments, the Zipf pattern is still present on current data center traffic.
Such characteristic favors flow caching for memory scarce programmable dataplanes.
However, we notice that short-lived flows dominate the trace.
Therefore, the chosen caching policy algorithm needs fast reaction time in order to adapt to traffic changes.
In addition, such temporal traffic characteristic will possibly favor time-aware cache policies (e.g. LRU).
