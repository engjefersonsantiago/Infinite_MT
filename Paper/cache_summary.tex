...
Also, we confirmed the good performance of LRU-based policies with up-to-date data center traffic, approaching the OPT as the cache size increases.
...
However, we observe that the OWLFU performance is very close (or superior) to OPT for when comparing the size weighted hit ratio.
...
The $SF$ scalability experiments attempt to mimic for realistic caching scenarios by considering a slower cache reaction time.
For more realistic scenarios, as shown in Figure~\ref{fig:hit_ratio_sf100} and Figure~\ref{fig:weighted_hit_ratio_sf100}, the performance of the OWLFU is very close to OPT.
This is due to the fact that speculatively assumes that the just inserted entry may remain in cache.
OPT makes no such assumption. As $SF$ increases, inserted entries there are not re-referenced may cause cache trashing due to the slower reaction time.
...
Figure~\ref{fig:promo_weighted} shows a noticeable increase (>10\%) in the size weighted hit ratio for when a random-based eviction policy is implemented.
