...
These policies were combined with other three cache eviction policies: random, LRU, and OWLFU.
...
\citeauthor{Miao:2017} have found that the software overhead was related to the CPU load for hash calculations, not the in the PCIe CPU-switch communication.
However, a match table cache scheme have more stringent requirements in terms of match table update rate considering the observed flows lifetime.
...
This is due high-speed links in data center networks and the fast changing nature of data center network traffic; therefore, a slow control plane interaction must be minimized.
However, candidates migration detection in the data plane can only be precisely detected for exact match rules due to shadowing effect in ternary and LPM rules.
...
However, a N-level cache can be generalized as a (N~$-$~1) independent 2-level caches.
Thus, the conducted 2-level cache performance investigation is relevant for setting a upper bound hit ratio analysis, even though we agree that a full caching system may expose other limitations, such as multi-level control plane interaction.

In our simulator, all table update metrics (communication, table insertion times, CPU load) are enveloped in an unified $SF$ metric.
Although $SF$ attempts to mimic performance gaps, in a real hardware it may not be realistic.
...
Finally, our simulator does not support batching for neither table insertion nor when gathering eviction/promotion policy counters.
