...
%% article.
\begin{abstract}
The P4 language and modern programmable dataplanes have redrawn the networking landscape as they allow fully data path programming in SDN environments.
...
However, modern programmable dataplanes lack in memory capacity for implementing match-tables.
Recent research has suggested mixing up different programmable dataplane devices to increase memory capacity.

In this work, we studied a cache hierarchy scheme in heterogeneous programmable dataplanes to emulate a single "virtually infinite" match table.
Our analysis started by re-characterizing an up-to-data data center trace.
Then, following our observations we derived caching premises for match-action caching.
Finally, we developed an open-source simulator to evaluate the viability of our proposed caching scheme.
...
\keywords{programmable networks, flow caching, cache policy, P4}
